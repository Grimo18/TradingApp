"""Interfaccia grafica CustomTkinter V7.0 (Istituzionale)."""
...
class TradingApp:
    def __init__(self):
        ...
        self.app.geometry("1200x900") # Leggermente ingrandita per i nuovi menu
        self.app.minsize(*config.APP_MIN_SIZE)
        self.app.configure(fg_color=config.COLOR_BG)

        self._setup_fonts()
        self._build_layout()
        self._log_to_terminal("TERMINALE V7.0 PRONTO. In attesa di selezione modalità...")
    ...
    def _build_layout(self):
        ...
        ctk.CTkLabel(header, text="V7.0 - Cloud AI + Kill Switch & Telegram", font=self.subtitle_font, text_color=config.COLOR_TEXT_SUBTLE).grid(row=1, column=0, sticky="w", padx=30, pady=(0, 20))
        ...
        card_left.grid_columnconfigure((0,1), weight=1)

        ctk.CTkLabel(card_left, text="PARAMETRI E WATCHLIST", font=self.section_font, text_color=config.COLOR_ACCENT).grid(row=0, column=0, columnspan=2, sticky="w", padx=20, pady=(15, 10))
        ...
        self.entry_capitale.grid(row=1, column=0, columnspan=2, sticky="ew", padx=20, pady=5)

        watchlists = [
            "EURUSD, GBPUSD, USDJPY, AUDUSD, USDCAD, BTCUSD, ETHUSD, AAPL.OQ, MSFT.OQ, NVDA.OQ, TSLA.OQ, AMZN.OQ, META.OQ, GOOGL.OQ, NFLX.OQ, AMD.OQ, INTC.OQ, QCOM.OQ, CSCO.OQ, PEP.OQ, COST.OQ, SBUX.OQ, PYPL.OQ, MU.OQ, TXN.OQ, INTU.OQ, AMAT.OQ, CMCSA.OQ, GILD.OQ, MDLZ.OQ", 
            "NVDA.OQ, TSLA.OQ, AAPL.OQ, MSFT.OQ, META.OQ, AMZN.OQ", 
            "EURUSD, GBPUSD, USDJPY, USDCAD, AUDUSD",
            "BTCUSD, ETHUSD",
        ]
        
        self.combo_ticker = ctk.CTkComboBox(card_left, values=watchlists, height=40, fg_color=config.COLOR_PANEL, border_width=0, button_color=config.COLOR_ACCENT)
        self.combo_ticker.grid(row=2, column=0, columnspan=2, sticky="ew", padx=20, pady=5)
        self.combo_ticker.set(watchlists[0])

        # --- SEZIONE: RISK MANAGEMENT (KILL SWITCH) ---
        ctk.CTkLabel(card_left, text="RISK MANAGEMENT GIORNALIERO", font=self.section_font, text_color=config.COLOR_WARNING).grid(row=3, column=0, columnspan=2, sticky="w", padx=20, pady=(20, 10))
        
        self.entry_target = ctk.CTkEntry(card_left, placeholder_text="Target Profitto ($)", height=40, border_width=0, fg_color=config.COLOR_PANEL)
        self.entry_target.grid(row=4, column=0, sticky="ew", padx=(20, 5), pady=5)
        
        self.entry_loss = ctk.CTkEntry(card_left, placeholder_text="Max Perdita ($)", height=40, border_width=0, fg_color=config.COLOR_PANEL)
        self.entry_loss.grid(row=4, column=1, sticky="ew", padx=(5, 20), pady=5)

        # --- SEZIONE: TELEGRAM ---
        ctk.CTkLabel(card_left, text="NOTIFICHE TELEGRAM", font=self.section_font, text_color="#10b981").grid(row=5, column=0, columnspan=2, sticky="w", padx=20, pady=(20, 10))
        
        self.entry_tg_token = ctk.CTkEntry(card_left, placeholder_text="Telegram Bot Token", height=40, border_width=0, fg_color=config.COLOR_PANEL)
        self.entry_tg_token.grid(row=6, column=0, columnspan=2, sticky="ew", padx=20, pady=5)
        
        self.entry_tg_chat = ctk.CTkEntry(card_left, placeholder_text="Tuo Chat ID", height=40, border_width=0, fg_color=config.COLOR_PANEL)
        self.entry_tg_chat.grid(row=7, column=0, columnspan=2, sticky="ew", padx=20, pady=5)

        self.bottone_avvio = ctk.CTkButton(card_left, text="▶ AVVIA", height=50, font=self.section_font, fg_color=config.COLOR_ACCENT, hover_color=config.COLOR_ACCENT_HOVER, command=self._on_start_btn)
        self.bottone_avvio.grid(row=8, column=0, columnspan=2, sticky="ew", padx=20, pady=(30, 20))

        # --- PANNELLO DESTRO: PORTAFOGLIO E TERMINALE ---
